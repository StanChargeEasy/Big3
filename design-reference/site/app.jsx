...
/* ---------- HERO ---------- */
function Hero({ t, onReveal, media, layout, formMode, introMode, onOpenModal, showcase, blend }) {
  ...
  return (
    <section className={heroCls}>
      ...
      <div className="b3-shell b3-hero-grid">
        {show ? (
          <React.Fragment>
            <div className="b3-hero-copy b3-hero-copy--top">{h1El}</div>
        ...
        <div className="b3-hero-copy b3-hero-copy--bot">
          {show && <div className="b3-hero-eyebrow-below">{eyebrowEl}</div>}
          {introMode === 'full' && (
      ...
      default: return (
        <React.Fragment>
          <Hero t={t} onReveal={reveal} media={tw.heroMedia} layout={tw.heroLayout}
            formMode={tw.formMode} introMode={tw.introMode} onOpenModal={() => setModalOpen(true)} showcase={tw.vxHero} blend={tw.vxBlend} />
  ...
  return (
    <div className={`b3-app${tw.vxDark ? ' vx-dark' : ''}${tw.vxTriad ? ' vx-triad' : ''}${tw.vxJournal ? ' vx-journal' : ''}${tw.vxType ? ' vx-type' : ''}`} data-head={tw.headingFont} style={rootStyle}>
      ...
      <TweaksPanel>
        ...
        <TweakToggle label="Headline ↔ Video verschmelzen" value={tw.vxBlend}
